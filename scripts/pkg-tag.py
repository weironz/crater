...
为什么一份蓝图只能发一个版本:`crater pkg push` 目前**没法设 build 期参数**,
所以同一份蓝图做不出别的版本的包。硬发一个别的 tag,内容仍然是默认版本 ——
那正是 D-159 那个错:索引声称存在 4.40.5,装下去是 4.44.3。等 `pkg push`
支持 `--set` 之后,这里要跟着改成"tag 由传入的版本决定"。
