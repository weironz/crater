...
| 文件 | 内容 |
|------|------|
| `component-schema.md` | 组件描述文件完整 schema（动作原语全集、参数规范） |
| `spec-schema.md` | `crater.yaml` 顶层 schema（inventory + components + 全局 + 预留 AI/offline 字段） |
| `offline-format.md` | 离线包格式定型（sealos OCI vs KubeKey tar 对比与选型） |
...
3. **新会话先读文档**：上下文窗口可能丢失，新会话先读 `requirements.md` + `decisions.md` 恢复全部背景。
