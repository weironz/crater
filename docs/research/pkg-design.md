...
目录页分「工作区 / 远端源」两个标签。远端源那一页**只做一件事:把包弄进
工作区**;弄进来之后它就是一张普通的本地蓝图,参数表单、机群对账、建任务、
plan 闸门一步不变 —— 所以那一页没有第二套表单逻辑,也就没有"两套逻辑迟早
跑偏"的问题。

浏览器实测:三张远端卡片(机群需求印在卡上)→ 点 rustfs → 卡片标"✓ 已在
工作区"并变淡 → 切回工作区标签,rustfs 已是本地卡片 → 点开,参数表单原样
出现 → 选机群,`✓ storage:1 台(需要 ≥1)` 对账照常。

两处刻意的取舍:

- **数据全部来自本地缓存的索引,不连网。** 于是这一页在断网机房里照样能开,
  而"要不要联网"是使用者按「同步索引」时的一个明确决定。
- **默认瘦拉**,弹窗把选择讲清楚:只拉蓝图(几十 KB,在线部署够用)还是
  连物料一起拉(可能几百兆,离线现场用)。默认那个是多数人要的那个。
