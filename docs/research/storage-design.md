...
> **crater 本身不依赖 git。** 这一节讲的是"你该怎么管理这些文件",不是
> crater 的运行时要求 —— 二进制里零处调用 `git`,依赖里没有 git2/gix,
> 发布的镜像也不含 git。工作区就是一个放 YAML 的目录,不 `git init` 照样跑。
> 初稿把这两件事写混了,在此更正。
>
> 将来若做"UI 写入自动 commit",也应当是**检测到才做**:工作区是 git 仓库
> 且 `git` 命令在场就提交,否则静默跳过并在启动横幅里说明。一个为了记版本
> 而强制你装 git 的部署工具,与 agentless、静态单二进制、闭包一个文件带走
> 一切的气质自相矛盾。
