...
### A7 · "看起来在正常工作"的三种失败(真机验证实录)

k8s HA 的建、升、销一轮跑下来,挖出的缺陷有个共同形态:**它们都不报错**。
单测与假目标覆盖不到,因为假目标只会回答你问它的问题;而这三个 bug 的要害
恰恰是**问错了问题**。

| 缺陷 | 表面现象 | 真相 |
| --- | --- | --- |
| `check` 检验了前序步骤建立的前提 | 报告一片 `ok` | 三步升级全被跳过,集群一点没升 |
| `material_digest` 对二进制返回 `None` | diff 说"无变化" | 二进制永远不会被替换 |
| `write_file` 拼 78MB 单条命令 | 进程在忙(100% CPU) | O(n²) memcpy,零网络,永不完成 |

三条各自的教训:

**1. `check` 只能检验本步自己的效果。** 不能检验一个由前序步骤建立的前提 ——
否则这一步永远被跳过,而且是静默跳过。前提该由步骤顺序或 `deps` 表达。
引擎无法静态判断这件事,但可以**把可疑之处摆到眼前**:舞的报告现在把
"因 check 已满足而未执行"的步骤单独点名。

**2. 内容寻址不能挑食。** 摘要计算若要求 UTF-8,就等于在**离线闭包最主要的
载荷(二进制)**上失效。更隐蔽的是它在资源路径上有 `upstream_changed` 兜底,
只在 procedure 里(那里 `upstream_changed` 恒为 false)才暴露 —— 于是"平时
都好好的,只有升级不生效"。

**3. "能跑通"不等于"跑得动"。** 文档注释写着 streams,实现却是一次性拼装;
小文件上完全正常,58MB 才显形。**任何"把数据拼进命令行"的实现都必须有块大小
上限**,否则它只是把失败推迟到数据变大的那天。

### 附:一次错误归因的记录

上面那条 `write_file` 拼 78MB 命令的缺陷是**真的**(trait 默认实现确实如此),
但它**不是**真机上"推二进制卡住"的原因 —— SSH executor 覆盖了这个方法,
走的是单通道 `cat > path` 原始字节流,从不拼命令。

真正的原因平庸得多:**整轮验证都在用 `cargo run`(debug 构建)**。
同一操作,58MB 推到一台机器:

```
debug    real 17.3s   user CPU 26.2s
release  real  2.7s   user CPU  1.2s     ← 6.4× 墙钟,21× CPU
```

russh 的加密与哈希跑在无优化代码上慢一个数量级,表现出来就是"100% CPU、
网络几乎空转" —— 与真正的死循环**在指标上无法区分**。

两条教训:

1. **性能类判断必须在 release 上做。** debug 构建的 crypto/hash 慢到足以把
   "慢"误判成"挂"。本次验证里所有耗时数字(10 分钟的首次部署等)都要按这个
   倍率重新理解。
2. **先确认是哪份实现在跑,再归因。** 我看到 trait 默认实现有缺陷就下了结论,
   而实际执行的是 impl 里的覆盖版本。`grep -rn "fn write_file"` 只要早做一步,
   就不会有这次误判。

方法论上最该记住的是**诊断路径**:三个现象——CPU 满、网络空、目标机零会话——
任何单独一个都会指向错误的结论(以为在计算 / 以为网络慢 / 以为已经结束),
三个凑齐才唯一地指向"发送端自己在原地打转"。**测量要成组,不能只看一个指标。**
