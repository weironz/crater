...
### A3 · 篇幅规范:多长必须拆(v1 即生效,lint 强制)

> 用户裁定:这是地基,前期必须定死。原则:**篇幅是气味不是罪** —— 全部规则是
> W 级警告(默认显示,CI `--strict` 下升级为失败),补救命令由 lint 直接给出。
> 行数一律按**内容行**计(非空、非注释)—— 注释是好东西,阈值不能逼人删注释。

| 码 | 规则 | 阈值 | 补救(lint 原文给出) |
|---|---|---|---|
| W430 | 单个顶层节过长 | 节内容行 > **80** | `crater fmt --split <节名>`(A1 外置) |
| W431 | 根文件过长且未分节 | 根文件内容行 > **200** 且 `parts` 为空 | 外置最大的节 |
| W432 | 蓝图整体过大 | 合并后内容行 > **400** | **拆蓝图 + stack(A2)** —— 这不再是篇幅问题,是职责问题;lint 按 `on:`/`when:` 聚类给出建议切分点 |

阈值的依据(不是拍脑袋):
- **80 行 ≈ 两屏**:一个节应当一次滚动读完 —— 3am 评委的"就地读完"标准;
- **200 行 ≈ 90 秒理解上限**:evaluator 实测事故复盘的认知预算;
- **400 行 ≈ 职责边界气味**:超过它的蓝图几乎必然在装两个东西。kubespray 映射的
  六个蓝图(§8 A2)每个都在 400 以下 —— 阈值反过来校验了组合设计的合理性。

配套纪律:
1. 阈值**不可在文件内豁免**(没有 `# lint-disable`)—— 团队级调整走 lint 配置,
   豁免要在 code review 里可见,不能藏在文件里;
2. W432 触发时,`crater plan` 摘要行附带提示(作者看不见 lint 也躲不过);
3. 三条规则进 `crater types` 同源注册表,`crater explain size` 可查全文。


### A4 · 表达式层裁定:留用 CEL,按位置限权(用户裁定,推翻 §3.1/§3.2)

> 触发:用户审阅时问「我们放弃了 CEL,自己发明了一门语言?」并裁定
> 「重新发明一门语言风险太大,用回 CEL」。本条**推翻**面板在 §3.1/§3.2 的
> 微文法方案,但**保留**面板真正的发现。

#### 为什么面板要换掉 CEL,以及为什么那个诊断只对了一半

面板的证据是实打实的:D-115 的 blueprint 里出现了

```yaml
cmd: kubeadm init ${params.ha ? "--upload-certs" : ""}
```

逻辑进了字符串 —— 本设计声称要消灭的东西。面板由此判定 CEL 有罪:它有三元、
有算术、有函数、有拼接,**给了作者足够的绳子**。

诊断对了一半。真正的病灶不是"CEL 能写三元",而是**我们允许完整 CEL 出现在值位置**。
条件属于 `when:`,值位置只该有名词 —— 这是**位置**问题,不是**语言**问题。
换一门更小的语言能治标,代价是自造 parser、零生态、边界由我们画(画错就要改语言,
而那正是 Ansible 引入 Jinja 的历史路径)。

#### 裁定

| | 决定 |
|---|---|
| 表达式引擎 | **CEL**(`cel` 0.14),不自造语言 |
| `when:` 位置 | **完整 CEL**:比较、布尔运算、`has()`、`size()`、`in` 皆可 |
| `${}` 插值位置 | **只允许纯引用** —— 语法树必须是"标识符 + 点路径",出现任何运算符、三元、函数调用、下标、字面量拼接一律 error[E310] |
| 校验方式 | lint 期检查(零连接),不是运行期 |

纯引用文法(用于 `${}` 内,由 lint 强制,**不是新语言** —— 它是 CEL 的一个子集):

```
pure-ref := ident ("." ident)*
```

E310 的报错直接给结构化改写(§3.4 的 flags 方案),而不是只说"不许这么写"。

#### 面板成果的存废清单

**保留(与表达式引擎无关,这些才是面板的主要价值):**

- §3.4 **结构化 flags** —— `cmd: {argv, flags: [{name, value?, when?}]}`。这是"条件拼 flag"
  的真正解药,它从来不依赖微文法:`when:` 里写什么语言都行,关键是**条件成了条目的属性**,
  而不是字符串里的三元。`flags[].name` 禁插值 ⇒ lint 仍可静态枚举命令全部展开。
- §2.4 `cast` 选角表、§2.5 selector 微语法(这是**定址**的小文法,不是通用表达式语言,
  自造成本低、边界清晰,保留)
- §2.1 `fleet` 组契约(`min:`)、§2.2 无 default 即 required / `doc` / `phase` / secret 污点流
- §2.3 materials `source.by` 开关表变体
- §5 可发现性三件套(`crater types` 字段卡、自特化 JSON Schema、四要素报错)
- plan 的两条规范义务(解释缺席、标注传导)
- §8 A1 约定式分节 / A2 Stack 层 / A3 篇幅阈值

**废止:**

- §3.1 的自造插值文法 → 改为"CEL 子集 + 位置限权"(本条)
- §3.2 的条件子句微文法(`is set` / `is not empty` / 仅 `and`)→ 回归 CEL。
  对应写法:`is set` → `has(params.x)`;`is not empty` → `size(...) > 0`;
  `or` 直接用 CEL 的 `||`(§6 拒绝 `or` 的理由是"无括号双连接词易误读",
  CEL 有括号,该理由不成立)
- §3.3 `join` 构造器 → CEL 有 `.join()` 之类的列表操作,不必自造

#### 代价与承认

放弃微文法也放弃了它的两个真实好处,如实记下:

1. **更强的静态类型检查** —— 微文法能保证 `==` 两侧类型匹配、枚举值域穷尽;
   CEL 的静态检查只到"根变量是否声明、函数是否在白名单"。这是净损失。
2. **`when:` 的可读性** —— `params.cp_endpoint is set` 确实比 `has(params.cp_endpoint)`
   更接近自然语言。这是净损失。

换回的是:零自造语言维护成本、CEL 的公开规范与生态、作者的既有认知(k8s 用户)。
**这笔交易由用户裁定,理由是长期维护风险 —— 记录在此,以便将来重新评估。**


### A5 · `on:` 键名的 YAML 1.1 风险(schema 实测发现,待裁定)

> 发现方式:实现 `crater schema` 后用真实校验器(Python jsonschema + PyYAML)跑
> k8s blueprint,29 条错误里有 **26 条**长这样:`{True: 'role.controlplane'}` ——
> 键 `on` 被解析成了布尔 `true`。

**事实**:YAML **1.1** 把裸词 `on / off / yes / no / true / false` 当布尔;
YAML **1.2** 只认 `true/false`。crater 自己用 serde_yaml(1.2),**解析无误**;
但 PyYAML、许多 CI 工具、部分编辑器插件仍是 1.1 —— 它们会把我们的元字段
`on:` 读成 `true:`。
...
| 方案 | 代价 | 收益 |
|---|---|---|
| **A. 保持 `on:`** | 用 1.1 工具的人踩坑;文档需醒目警告 | 语义最贴切,面板首选,与 GitHub Actions 的 `on:` 同名(它也有同样问题) |
