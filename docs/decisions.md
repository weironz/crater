...
### D-157:版本范围 —— 补上 helm #11000 点名的那个用例

**由来**:helm 的 issue #11000(2022-05 开,36 条评论,至今 OPEN)说 OCI 模式
下没法发现版本。查的时候顺手对着**报告人自己的仓库**试了一遍:

    crater pkg tags ghcr.io/rowi1de/graphql-reactive/graphql-reactive
    → 0.1.18 / 0.1.17 / 0.1.16 …

发现这半我们本来就有(`tags/list` 是 OCI 唯一的内容发现端点,D-123 用的就是
它)。**缺的是范围**:issue 里明确列的 `--version '0.0.*'`,我们只能精确匹配。

**支持的写法**:`4.*`、`^4.44.3`、`~4.44.3`、`>=4.44`、`<5`,逗号或空格分隔
表示**全部满足**。不支持 `||` —— 那要一棵表达式树,而至今没有一个真实需求,
加了就要一直背着。

**两条路都通**,而且解析方式不同:

- 包名(`yq:4.*`)→ 索引里已有全部版本,直接筛
- 直连引用(`oci://reg/ns/yq:4.*`)→ **问 registry 的 `tags/list`**,再挑最高
  的合格者。这条正是 issue 里那个原例,而它**不需要索引** —— 发现"有哪些
  版本"是 OCI 自带的能力,发现"有哪些包"才不是。

**不引 `semver` crate,建在已有的 `pkg::semver_key` 上。** 理由是真实 registry
上的 tag 不都是严格 semver:`1`、`4.40`、`v1.2`、`latest`、`probe` 都常见,
而 `semver::Version::parse` 对前两个直接失败。`semver_key` 是宽松的,认不出
的段记 -1 —— 结果是不认识的 tag **排在后面**,而不是让整次解析崩掉。

**几处刻意的选择,每一处都配了测试:**

- **精确匹配比原字符串,不比解析后的数。** `4.44` 与 `4.44.0` 在 registry 上
  是两个不同的 tag,拉下来是两份不同的字节。用 key 比会让它们相等。
- **`^0.2.3` 不放行 `0.3.0`。** semver 的规矩:0.x 里次版本就是破坏性边界。
  写错这条会让人在 0.x 的库上自动升到一个不兼容的版本。
- **前缀通配按段比,不按字符串比。** 字符串前缀会让 `4.4.*` 命中 `4.44.0`。
- **`best()` 自己排序**,不假设入参有序 —— `tags/list` 的顺序是 registry 说了
  算,Docker Hub 与 zot 给的就不一样。依赖调用方排过序,是一种换了 registry
  之后才暴露的错。
- **精确 tag 不去列版本。** 少一次往返只是顺带;真正的理由是有些仓库只给
  pull 权限、列不了 tag,精确引用在那种仓库上必须照样能用。
- **写错的范围当场报错**,不退化成"精确匹配这串字符"。后者的表现是"仓库里
  没有 `^` 版",一条把人引向仓库而不是引向拼写的错误。实测 `yq:^` 现在报的
  是"`^` 后面要跟一个版本号"。

**最容易错的一处**:从引用里切 tag 只能看**最后一段**里的冒号。
`localhost:5000/ns/yq:4.*` 的第一个冒号是端口 —— 按第一个冒号切会得到仓库
`localhost`、tag `5000/ns/yq:4.*`,然后报"localhost 上没有匹配的版本",把人
引向 registry 而不是这行代码。单独一条测试钉住。

**端到端**(本地 registry,推了 4.9.0 / 4.10.0 / 4.44.1 / 4.44.3 / 5.0.0):
`4.*`→4.44.3(不是 5.0.0 也不是字典序的 4.9.0)、`^4.44`→4.44.3、
`~4.44.1`→4.44.3、`>=4.10, <4.44`→4.10.0、`0.0.*`→报错并列出现有版本。
包名与直连引用两条路各跑一遍,结果一致。
...
### D-159:我发了两个不存在的版本 —— 索引必须由机器从 registry 生成

**是我造的错,而且已经发出去了(私有,无人拉过,已撤)。**

D-158 里为了验版本范围,我把**同一份内容**推成了三个 tag:

    ghcr.io/weironz/crater/yq:4.40.5
    ghcr.io/weironz/crater/yq:4.44.3
    ghcr.io/weironz/crater/yq:4.45.1

然后用 `pkg index` 从 registry 生成索引、提交、发布。索引照单全收,于是它
开始声称存在 4.40.5 和 4.45.1 —— **而谁装它们都会装到 4.44.3**。

`pkg inspect` 三个 tag,`version 默认 4.44.3` 一模一样。已删掉那两个 tag,
索引重生成为 1 个版本。

**根因不是手滑,是两件事凑在一起:**

1. **`crater pkg push` 没法设 build 期参数。** yq 的 `version` 是
   `stage: build`,而 `pkg push` 不接受 `--set` —— 也就是说,一份蓝图**只能**
   发出它 `params.version.default` 那一个版本。我以为 tag 是个自由标签,
   实际它必须与内容一致。
2. **索引是手工生成、手工提交的。** 我用一个错的 registry 状态生成了一份
   忠实的索引。索引没错,它忠实反映了 registry —— 错的是 registry,而没有
   任何东西挡在中间。

**修法有三层:**

- **撤掉假 tag**(已做)。
- **索引改由 CI 从 registry 重建**(`.github/workflows/packages.yml`)。
  它不接受"应该有什么"的输入 —— 判据只有一条:**索引里有什么,以 registry
  上有什么为准**。手工那条路的问题不在于会出错,而在于**出错时没有第二双
  眼睛**。
- **tag 由蓝图自己决定,不由人填**(`scripts/pkg-tag.py`)。规则:
  `params.version.default` 有就用它,否则用蓝图的 `version:`。
  实测 docker→27.3.1、yq→4.44.3、zot→2.1.17、rustfs→1.0.0-beta.7、mysql→1。
  写成可本地跑的脚本而不是埋在 workflow 的 YAML 里 —— 埋进去的逻辑只能靠
  推一次 CI 来验,而那正是"验证很贵所以不验"的开始。

**为什么不用蓝图的 `version:` 当 tag**:那是**蓝图自己的修订号**,库里全是
`1`。按它发,所有包都挤在 tag `1` 上,"yq 的哪个版本"就没人能回答了。
`pkg push` 其实一直在提醒这件事(「tag `4.44.3` 与蓝图 version `1` 不同」),
而我把这条提醒当成了噪音 —— **它是对的,是我没听**。

**留下的真缺口(未做)**:`pkg push` / `pkg build` 不支持 `--set`,所以库里
一份蓝图对应且仅对应一个可发布版本。要发 yq 4.45.1 就得改蓝图的默认值。
这不是伪需求 —— helm 一个 chart 能发多版本正是靠 `--version`。另开 issue。

**这条与本仓库反复出现的那一课是同一条**,只是这次的载体是包:
**没有第二双眼睛的东西会烂**,而它烂掉时看起来一切正常。

### D-160:发新版本的完整流程,以及它暴露的一个真债

**起因是一个操作问题**:"做了新版本的 yq,索引怎么更新"。走了一遍真的,
四步:

1. 改蓝图的 `params.version.default`(**现在只能这样** —— `pkg push` 不支持
   `--set`,见 issue #25)
2. `crater lint` 过一遍
3. `crater pkg push`,tag 由 `scripts/pkg-tag.py` 从蓝图导出,**不手填**
4. `crater pkg index oci://<ref>` 从 registry 重建索引

**旧版本自动留着** —— 索引读的是 registry 的 `tags/list`,不是蓝图。这是
"索引由机器从 registry 生成"(D-159)顺带的好处:发新版不用惦记别把老版
挤掉。

**但验的时候发现装不上老版本。** `crater apply yq:4.44.3` 跑完,机器上还是
4.53.6。

**这不是 bug —— crater 做对了,而且把原因和出路都打出来了:**

    1 个物料没声明 `sha256:`,这次又没有闭包 —— 目标机上那份是不是旧版本,
    crater 判不出(D-135)。计划里它们会是 `?`,不是 `~ 将修改`,
    apply 也不会动它们。

它解析对了版本、换了 app 文件的指向、报的是 `?1` 而不是假装成功。**真正的
债在 `library/yq` 的蓝图上**:物料没写摘要,所以判不出"目标机上这份是不是
这一版",而判不出就不动 —— 这正是三态诚实该有的样子。

**修法不是"给它配一个 sha256"。** yq 的 URL 带 `${substrate.arch}`,而
`sha256` 是单值 —— 一个摘要只对一个架构成立,另一个架构上会落地即失败。
物料的**变体机制**(同名多条 + `when:`)正是为此存在:
...
摘要取自上游 `checksums` 的第 18 列(SHA-256),并与本地实算逐字比对 ——
只信一边就等于没核对。

**已发布的 4.44.3 也重推了一遍**(带它自己那一版的摘要)。理由:索引宣传的
每一个版本都该能钉住,否则就是 D-159 的小号版本 —— 广告里有,实际用不了。

**前后对比,同一台干净容器:**

| | 改之前 | 改之后 |
| --- | --- | --- |
| `apply yq:4.44.3` | 计划 `?1`,装完仍是 4.53.6 | 计划 `~1`,真装成 4.44.3 |
| 重复 apply | `?1`(判不出) | `✓1`(核对通过) |
| `verify yq` | 「无法核对」+ 非零退出 | 「✓ 现实符合期望」 |

**留下一条给作者的纪律**:摘要与 `version` 必须**同时**改。版本变了摘要就
不对,而不对的表现是落地失败 —— 那正是内容寻址该有的样子(与 library/k8s
同一条)。

**顺带一个没查的疑点**:`library/k8s` 的物料 URL 也带 `${substrate.arch}`,
却只配了一个 `sha256`。按上面的推理,它在 arm64 上会摘要不符。没有 arm64
机器验证,先记在这里。

### D-161:`pkg push --set` —— 一份蓝图发多个版本(issue #25)

**`--set` 就是变量渲染,和 helm 一个意思。** 只是 crater 的参数分两个阶段:
打包时给的覆盖 `stage: build`(定死在包里),部署时给的覆盖 `stage: apply`。
分两段的理由很实在 —— `version` 决定**下载哪个 URL**,而物料要在打包时就抓
下来烤进闭包,部署时才问就太晚了。

**覆盖值烤进包里的那份蓝图文本,不是另存一处。**

包解开是一个目录,里面那份蓝图就是"这个包是什么"的正身:人读它、改它、
`git diff` 它,`crater apply -f <那份蓝图>` 也能直接跑。覆盖值若存在别处
(包的 config、一份 overrides),解出来的蓝图就会**写着一套装的是另一套** ——
这个仓库反复栽在这类事上(D-159 的假版本号、D-154 的空转标志)。

**逐行改而不是 serde 读回再写出**:后者抹平注释、空行、键序,而库里的蓝图
大半价值在注释上。认三种写法(内联 flow map / 块式 / 裸标量),都不匹配就
**报错**,绝不静默跳过 —— "`--set` 没报错但也没生效"的包会一路发到索引里。
12 条单测覆盖:注释存活、引号风格跟随原值(`"1.2"` 不能变成浮点数 1.2)、
引号内的逗号不切、拼错参数名给建议、末尾换行保持(差一个换行整包 digest 就变)。

**做的过程中撞出一个更根本的问题:摘要不能参数化,而 URL 能。**

D-160 刚给 yq 的物料钉上 per-arch 摘要;而 `--set version=4.44.3` 换了 URL、
摘要没换 → 落地摘要不符。lint 里其实早写着这对矛盾(「URL 里插值了
`${params.version}` 时,写死一个 sha256 反而是错的」),给的出路是 `--closure`。

**第三条出路是让摘要也可插值。** `Material.sha256` 从 `Option<String>` 改成
`Option<Value>`,和 `source` 走同一条渲染。理由:两者本就是一对(某个版本的
字节 + 那份字节的摘要),只让其中一个可参数化,等于让它们**必然**走散。

**反证做了**:把渲染退回原样透传,新测试立刻红,报的正是实测撞见的那句
`期望 ${params.sha_amd64}` —— 漏渲染时不会崩,而是把字面量当成期望摘要去比。

**验收(#25 的原文条件)**:同一份 `library/yq`,`--set` 推出 4.44.3、默认推出
4.53.6;干净容器里各装各的版本,来回切换都是 `~1`(会改)并真的换掉,幂等是
`✓1`(能核对)。源文件一个字没动。

**又踩了一次 D-142**:第一次验证失败(报 `期望 ${params.sha_amd64}`),差点
去改代码 —— 真因是我在测**改动之前编的那个 musl 二进制**。这个坑在本仓库
已经是第三次。`touch` 源文件强制重编之后,同一条命令直接就过了。

四道闸门全绿,16 套 628 个用例(新增 14 个)。

### D-162:`library/yq` 变成一个包的完整生命周期走查

**要的东西**:拿最小的那个包,把构建 → 推送 → 拉取使用 → 离线 → 升降级
整条路走一遍,写成文档。

**做法上只有一条纪律:每一段输出都是真跑出来的。** 起本地 registry、推在线包
与离线包、`pkg pull` 摊回文件、`pkg save` 导 tar、在 `--network none` 且
**连 curl 都没装**的容器里 `pkg load` + 装上 —— 然后把终端里的字抄进文档。
照着 `--help` 编输出,和 README 里那条 404 的安装命令是同一种病。

**而这么做当场抓到一个真 bug。**

`pkg load` 成功后打的提示是:

    闭包完整,不用连网。`crater install <ref> -i <机群>` 就能装。

**照它敲会失败。** 断网容器里实测:少了 `--full`,install 走瘦拉,物料仍然
按 URL 去下载,报 `下载失败(exit 127) no curl/wget on target`。

这条提示**恰恰是给断网现场看的**,而它在那里必然失败 —— 提示里漏一个开关,
等于把人送进一个与提示内容相反的结论。已改成:

    闭包完整,不用连网:
      crater install <ref> --full -i <机群>
      (`--full` 是关键 —— 少了它会去下载物料,断网现场必失败)

改完在同一个断网容器里重跑,照新提示走一次成。

**文档里 23 条命令逐条验过能解析**(D-154 那套判据:只解析不执行)。

**顺带记一个使用细节**:`pkg save` 从**本地 store** 导出。换了 `$HOME` 的干净
环境里它会报 `not in local store` —— 不是 bug,是"先得把包弄进 store"
(`pkg build` / `pkg push` / `pkg pull --full`)。文档里写明了。

四道闸门全绿,16 套 628 个用例。

### D-163:删掉 `pkg` 子命令组 —— 按 docker 的动词拉平

**用户的话是判据**:"类比docker不行吗"。

对的。crater 的**包**就相当于 docker 的**镜像**,而 docker 没有
`docker pkg save`。常用的动词被埋在 `pkg` 下、罕用的底层 store 操作占着顶层,
**方向是反的**。

    crater build <目录> -t <引用>      像 docker build -t
    crater push <引用> | <目录> <引用>
    crater pull <引用>
    crater save / load / tags / inspect / images / tag / rmi / gc

**冲突全部用 crater 已有的判据化解,没有引入新概念:**

| 冲突 | 判据 |
| --- | --- |
| `build` 出包还是出闭包 | `-t` vs `-o`,**恰择其一**,不给默认 —— 猜错的表现是"跑完了但产物不是要的那个" |
| `push` 推已有还是组装再推 | 位置参数**个数**(1 个 = 引用;2 个 = 目录 + 引用) |
| `pull` 包还是普通制品 | 先当包拉,判到"不是 crater 包"才退回收进 store |
| `inspect` 蓝图还是包 | 文件 vs 引用 —— 与 `apply` 同一套 |
| `save`/`load` | 直接用包语义。它与旧的**格式相同**(同一个 `import_oci_archive_rooted`),只是多了闭包完整性检查,**严格更强** |
| `images` / `pkg ls` | 并成一条:`images` 列全部,是 crater 包就多一列说明 |

**`pull` 的回退判据单独抽了个函数并写明理由**:判宽了会把真正的网络错误吞掉、
然后拿一条无关的路径重试;判窄了会让"拉一个容器镜像"变成一条报错。两个方向
都很糟,所以判据只认那一句"不是 crater 包"。实测两条路都验过:crater 包摊成
目录;普通容器镜像收进 store、不摊开。

**旧写法直接断,不留别名。** 理由:`pkg` 从来没进过正式发布版(v0.2.0 里
这一批改动一个都没有),现在断是零成本;留别名则要一直背着两套写法,而文档
和人的记忆都会漂。

**防漂移测试当场发挥作用** —— 改完枚举、还没动分组清单时它就报"清单里少了
index、tags;多了 pkg"。这正是 D-154 加它的理由。

清理了 **93 处**旧写法(代码 41、文档 52)。`docs/research/pkg-design.md` 与
`pkg-signing.md` **不改** —— 那是设计期的调研记录,改它等于篡改当时写了什么。
